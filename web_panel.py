...
{% if bots %}
    ...
    <!-- НОВЫЙ БЛОК: РЕДАКТОР ИМЕНИ -->
    <div class="control-block"><h3>Редактор имени</h3>
...
<script>
    ...
    function initializePanel() {
        ...
        Object.entries(BOTS).forEach(([ip, data]) => { let opt = document.createElement('option'); opt.value = ip; opt.innerHTML = formatBotName(ip, data); mainBotSelect.appendChild(opt); });
...
@app.route('/')
def index(): return render_template_string(HTML_TEMPLATE, bots_data=bots, image_files=image_files, audio_files=audio_files)

# НОВЫЙ API ДЛЯ ПЕРЕИМЕНОВАНИЯ
@app.route('/api/rename', methods=['POST'])
def api_rename():
    data = request.get_json()
    bot_id, new_alias = data.get('bot_id'), data.get('alias')
    if bot_id:
        aliases[bot_id] = new_alias # Обновляем словарь псевдонимов
        save_aliases(aliases) # Сохраняем в файл
        if bot_id in bots: bots[bot_id]['alias'] = new_alias # Обновляем и живые данные
...
def websocket_handler(ws):
    ...
    # Ждем первое сообщение с именем хоста
    try:
    ...
    except Exception: hostname = "timeout_host"
